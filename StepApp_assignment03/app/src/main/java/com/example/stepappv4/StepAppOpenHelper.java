...
public class StepAppOpenHelper extends SQLiteOpenHelper {
    ...
    public static Map<Integer, Integer> loadStepsByDay(Context context, String date) {
        // 1. Define a map to store the day and the number of steps as key-value pairs
        Map<Integer, Integer> map = new HashMap<>();
        ...
        // 3. Define the query to get the data
        //Cursor cursor = database.rawQuery("SELECT day, COUNT(*) FROM num_steps WHERE day = ? GROUP BY day ORDER BY day ASC", new String [] {date});
        Cursor cursor = database.rawQuery("SELECT day, COUNT(*) FROM num_steps GROUP BY day ORDER BY day ASC", null);

        // 4. Iterate over returned elements in the cursor
        cursor.moveToFirst();
        for (int index = 0; index < cursor.getCount(); index++) {
            Integer tmpKey = cursor.getInt(0);
            Integer tmpValue = cursor.getInt(1); // Count of steps for that day

            // Put the data from the database into the map
            map.put(tmpKey, tmpValue);
            Log.d("STEPS_MAP", "Hour/Day: " + tmpKey + " Steps: " + tmpValue);
