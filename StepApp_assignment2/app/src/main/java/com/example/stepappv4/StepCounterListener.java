...
public class  StepCounterListener implements SensorEventListener {
    ...
    public void onSensorChanged(SensorEvent sensorEvent) {
        ...
        switch (sensorEvent.sensor.getType())
        {
            case Sensor.TYPE_LINEAR_ACCELERATION:
                ...
                // we get the timestamp and convert it
            ...
            case Sensor.TYPE_STEP_DETECTOR:
                ...
                //Log.d("STEP_DETECTOR", "step made: " + step);
                countSteps(step);
                break;
        }
    }
    ...
    private void peakDetection() {
        ...
        // here we detect peaks
        for (int i = 1; i < valuesInWindow.size()-1; i++) {
            int forwardSlope = valuesInWindow.get(i + 1) - valuesInWindow.get(i); // in these two lines we compare the value with the next and previous steps,
            int downwardSlope = valuesInWindow.get(i) - valuesInWindow.get(i - 1);

            // then of course if the current value is bigger than both, it is indeed a peak, then we check if its bigger than the threshold, we are sure
    ...
    private void countSteps(float step)
    {
        ...
        // Log the step count
        Log.d("STEP_DETECTOR", "step made: " + accStepCounter);
        stepCountsView.setText(String.valueOf(accStepCounter));
